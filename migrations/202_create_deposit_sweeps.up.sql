...
CREATE INDEX idx_deposit_sweeps_pending_lookup ON deposit_sweeps(status, attempts, created_at) WHERE status = 'pending';
CREATE INDEX idx_deposit_sweeps_in_progress ON deposit_sweeps(status, updated_at) WHERE status = 'in_progress';
