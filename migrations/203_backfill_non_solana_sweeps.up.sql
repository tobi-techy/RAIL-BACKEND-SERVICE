...
INSERT INTO deposit_sweeps (id, deposit_id, user_id, source_chain, amount, fee_amount, status, created_at, updated_at)
SELECT
    gen_random_uuid(),
    d.id,
    d.user_id,
    d.chain,
    d.amount,
    NULL, -- fee_amount: not available for historical deposits
    'pending',
    NOW(),
    NOW()
FROM deposits d
