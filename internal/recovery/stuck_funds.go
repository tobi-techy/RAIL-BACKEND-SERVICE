...
func RecoverStuckBaseToSolana(ctx context.Context, cc *circle.HTTPClient, cr *chainrails.Client, p Params, logger *zap.Logger, out io.Writer) error {
	...
	// recovery): a deterministic UUID derived from wallet + requested amount + stable
	// destination address. intent.IntentAddress changes on every run (new ChainRails intent),
	// so using it would defeat deduplication on restart. p.ToSolanaAddr is the stable final
	// destination and ensures concurrent recovery attempts for different destinations never collide.
