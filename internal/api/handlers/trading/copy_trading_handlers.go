...
func (h *CopyTradingHandlers) ListConductors(c *gin.Context) {
	...
	if err != nil {
		...
		common.RespondInternalError(c, "Failed to list conductors")
		return
	}
...
func (h *CopyTradingHandlers) GetConductor(c *gin.Context) {
	...
	if err != nil {
		common.RespondBadRequest(c, "Invalid conductor ID")
		return
	}
	...
	if err != nil {
		if err.Error() == "conductor not found" {
			common.RespondNotFound(c, "Conductor not found")
			return
		}
		h.logger.Error("Failed to get conductor", "error", err)
		common.RespondInternalError(c, "Failed to get conductor")
		return
	}
...
func (h *CopyTradingHandlers) GetConductorSignals(c *gin.Context) {
	...
	if err != nil {
		...
		common.RespondInternalError(c, "Failed to get signals")
		return
	}
...
func (h *CopyTradingHandlers) CreateDraft(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		common.RespondBadRequest(c, "Invalid request body")
		return
	}

	if req.AllocatedCapital.LessThanOrEqual(decimal.Zero) {
		common.RespondBadRequest(c, "Allocated capital must be positive")
		return
	}
	...
	if err != nil {
		...
		case "conductor is not active":
			common.RespondError(c, http.StatusBadRequest, "CONDUCTOR_INACTIVE", "Conductor is not accepting new followers", nil)
		case "cannot copy your own trades":
			common.RespondError(c, http.StatusBadRequest, "SELF_COPY", "You cannot copy your own trades", nil)
		case "already following this conductor":
			common.RespondError(c, http.StatusConflict, "ALREADY_FOLLOWING", "You are already following this conductor", nil)
		default:
			if len(err.Error()) > 20 && err.Error()[:20] == "minimum allocation is" {
				common.RespondError(c, http.StatusBadRequest, "MIN_ALLOCATION", err.Error(), nil)
			} else if len(err.Error()) > 20 && err.Error()[:20] == "insufficient balance" {
				common.RespondError(c, http.StatusBadRequest, "INSUFFICIENT_BALANCE", err.Error(), nil)
			} else {
				common.RespondInternalError(c, "Failed to create draft")
			}
		}
		return
	}
...
func (h *CopyTradingHandlers) ListUserDrafts(c *gin.Context) {
	...
	if err != nil {
		...
		common.RespondInternalError(c, "Failed to get drafts")
		return
	}
...
func (h *CopyTradingHandlers) GetDraft(c *gin.Context) {
	...
	if err != nil {
		common.RespondBadRequest(c, "Invalid draft ID")
		return
	}
	...
	if err != nil {
		if err.Error() == "draft not found" {
			common.RespondNotFound(c, "Draft not found")
			return
		}
		if err.Error() == "unauthorized" {
			common.RespondUnauthorized(c, "Not authorized to view this draft")
			return
		}
		h.logger.Error("Failed to get draft", "error", err)
		common.RespondInternalError(c, "Failed to get draft")
		return
	}
...
func (h *CopyTradingHandlers) PauseDraft(c *gin.Context) {
	...
	if err := h.service.PauseDraft(c.Request.Context(), userID, draftID); err != nil {
		...
		common.RespondError(c, http.StatusBadRequest, "PAUSE_FAILED", err.Error(), nil)
		return
	}
...
func (h *CopyTradingHandlers) ResumeDraft(c *gin.Context) {
	...
	if err := h.service.ResumeDraft(c.Request.Context(), userID, draftID); err != nil {
		...
		common.RespondError(c, http.StatusBadRequest, "RESUME_FAILED", err.Error(), nil)
		return
	}
...
func (h *CopyTradingHandlers) UnlinkDraft(c *gin.Context) {
	...
	if err := h.service.UnlinkDraft(c.Request.Context(), userID, draftID); err != nil {
		...
		common.RespondError(c, http.StatusBadRequest, "UNLINK_FAILED", err.Error(), nil)
		return
	}
...
func (h *CopyTradingHandlers) ResizeDraft(c *gin.Context) {
	...
	if req.NewAllocatedCapital.LessThanOrEqual(decimal.Zero) {
		common.RespondBadRequest(c, "New allocated capital must be positive")
		return
	}
	...
	if err := h.service.ResizeDraft(c.Request.Context(), userID, draftID, req.NewAllocatedCapital); err != nil {
		...
		common.RespondError(c, http.StatusBadRequest, "RESIZE_FAILED", err.Error(), nil)
		return
	}
...
func (h *CopyTradingHandlers) GetDraftHistory(c *gin.Context) {
	...
	if err != nil {
		...
		common.RespondInternalError(c, "Failed to get history")
		return
	}
...
func (h *CopyTradingHandlers) ApplyAsConductor(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		common.RespondBadRequest(c, "Invalid request body: "+err.Error())
		return
	}
	...
	if err != nil {
		...
		case "user must have an existing Rail account":
			common.RespondError(c, http.StatusBadRequest, "NO_ACCOUNT", "You must have an existing Rail account to apply", nil)
		case "user is already a conductor":
			common.RespondError(c, http.StatusConflict, "ALREADY_CONDUCTOR", "You are already a conductor", nil)
		case "application already pending review":
			common.RespondError(c, http.StatusConflict, "APPLICATION_PENDING", "Your application is already pending review", nil)
		default:
			common.RespondInternalError(c, "Failed to submit application")
		}
		return
	}
...
func (h *CopyTradingHandlers) GetMyApplication(c *gin.Context) {
	...
	if err != nil {
		...
		common.RespondInternalError(c, "Failed to get application")
		return
	}

	if app == nil {
		common.RespondNotFound(c, "No application found")
		return
	}
...
func (h *CopyTradingHandlers) ListPendingApplications(c *gin.Context) {
	...
	if err != nil {
		...
		common.RespondInternalError(c, "Failed to list applications")
		return
	}
...
func (h *CopyTradingHandlers) ReviewApplication(c *gin.Context) {
	reviewerID, err := common.GetUserID(c)
	if err != nil {
		common.RespondUnauthorized(c, "User not authenticated")
		return
	}
	...
	if err != nil {
		common.RespondBadRequest(c, "Invalid application ID")
		return
	}
	...
	if !req.Approved && req.RejectionReason == "" {
		common.RespondBadRequest(c, "Rejection reason is required when rejecting")
		return
	}
	...
	if err := h.service.ReviewApplication(c.Request.Context(), applicationID, reviewerID, &req); err != nil {
		...
		if err.Error() == "application not found" {
			common.RespondNotFound(c, "Application not found")
			return
		}
		common.RespondError(c, http.StatusBadRequest, "REVIEW_FAILED", err.Error(), nil)
		return
	}
...
func (h *CopyTradingHandlers) CreateTrack(c *gin.Context) {
	...
	if err != nil {
		...
		case "user is not a conductor":
			common.RespondError(c, http.StatusForbidden, "NOT_CONDUCTOR", "You must be a conductor to create tracks", nil)
		case "conductor account is not active":
			common.RespondError(c, http.StatusForbidden, "CONDUCTOR_INACTIVE", "Your conductor account is not active", nil)
		default:
			if len(err.Error()) > 10 && err.Error()[:10] == "allocations" {
				common.RespondBadRequest(c, err.Error())
			} else {
				common.RespondInternalError(c, "Failed to create track")
			}
		}
		return
	}
...
func (h *CopyTradingHandlers) ListTracks(c *gin.Context) {
	...
	if err != nil {
		...
		common.RespondInternalError(c, "Failed to list tracks")
		return
	}
...
func (h *CopyTradingHandlers) GetTrack(c *gin.Context) {
	...
	if err != nil {
		common.RespondBadRequest(c, "Invalid track ID")
		return
	}
	...
	if err != nil {
		if err.Error() == "track not found" {
			common.RespondNotFound(c, "Track not found")
			return
		}
		h.logger.Error("Failed to get track", "error", err)
		common.RespondInternalError(c, "Failed to get track")
		return
	}
...
func (h *CopyTradingHandlers) GetConductorTracks(c *gin.Context) {
	...
	if err != nil {
		...
		common.RespondInternalError(c, "Failed to get tracks")
		return
	}
...
func (h *CopyTradingHandlers) UpdateTrack(c *gin.Context) {
	...
	if err != nil {
		...
		if err.Error() == "unauthorized to update this track" {
			common.RespondUnauthorized(c, "Not authorized to update this track")
			return
		}
		common.RespondError(c, http.StatusBadRequest, "UPDATE_FAILED", err.Error(), nil)
		return
	}
...
func (h *CopyTradingHandlers) DeleteTrack(c *gin.Context) {
	...
	if err := h.service.DeleteTrack(c.Request.Context(), userID, trackID); err != nil {
		...
		if err.Error() == "unauthorized to delete this track" {
			common.RespondUnauthorized(c, "Not authorized to delete this track")
			return
		}
		common.RespondError(c, http.StatusBadRequest, "DELETE_FAILED", err.Error(), nil)
		return
	}
