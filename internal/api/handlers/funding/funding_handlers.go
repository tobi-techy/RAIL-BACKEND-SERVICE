...
func (h *FundingHandlers) CreateDepositAddress(c *gin.Context) {
	...
	if err != nil {
		h.logger.Error("Failed to create deposit address",
			...
			"user_id", userUUID,
			"chain", req.Chain)
		common.SendInternalError(c, "DEPOSIT_ADDRESS_ERROR", "Failed to create deposit address")
		return
	}
...
func (h *FundingHandlers) GetFundingConfirmations(c *gin.Context) {
	...
	if err != nil {
		h.logger.Error("Failed to get funding confirmations",
			"error", err,
			"user_id", userUUID)
		common.SendInternalError(c, "CONFIRMATIONS_ERROR", "Failed to retrieve funding confirmations")
		return
	}
...
func (h *FundingHandlers) GetBalances(c *gin.Context) {
	...
	if err != nil {
		h.logger.Error("Failed to get balances",
			"error", err,
			"user_id", userUUID)
		common.SendInternalError(c, "BALANCES_ERROR", "Failed to retrieve balances")
		return
	}
...
func (h *FundingHandlers) GetTransactionHistory(c *gin.Context) {
	...
	if err != nil {
		h.logger.Error("Failed to get transaction history",
			"error", err,
			"user_id", userUUID)
		common.SendInternalError(c, "TRANSACTION_HISTORY_ERROR", "Failed to retrieve transaction history")
		return
	}
