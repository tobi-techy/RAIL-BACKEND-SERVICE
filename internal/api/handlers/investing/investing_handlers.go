...
func (h *InvestingHandlers) GetBaskets(c *gin.Context) {
	...
	if err != nil {
		...
		common.SendInternalError(c, "BASKETS_ERROR", "Failed to retrieve baskets")
		return
	}

	common.SendSuccess(c, baskets)
}
...
func (h *InvestingHandlers) GetBasket(c *gin.Context) {
	...
	if err != nil {
		common.SendBadRequest(c, "INVALID_BASKET_ID", "Invalid basket ID format")
		return
	}
	...
	if err != nil {
		...
		h.logger.Error("Failed to get basket",
			"error", err,
			"basket_id", basketID)
		common.SendInternalError(c, "BASKET_ERROR", "Failed to retrieve basket")
		return
	}
...
func (h *InvestingHandlers) GetOrders(c *gin.Context) {
	...
	if err != nil {
		h.logger.Error("Failed to get orders",
			"error", err,
			"user_id", userUUID)
		common.SendInternalError(c, "ORDERS_ERROR", "Failed to retrieve orders")
		return
	}

	common.SendSuccess(c, orders)
}
...
func (h *InvestingHandlers) GetOrder(c *gin.Context) {
	...
	if err != nil {
		common.SendBadRequest(c, "INVALID_ORDER_ID", "Invalid order ID format")
		return
	}
	...
	if err != nil {
		if err == investing.ErrOrderNotFound {
			common.SendNotFound(c, common.ErrCodeOrderNotFound, "Order not found")
			return
		}
		h.logger.Error("Failed to get order",
			...
			"user_id", userUUID,
			"order_id", orderID)
		common.SendInternalError(c, "ORDER_ERROR", "Failed to retrieve order")
		return
	}

	common.SendSuccess(c, order)
}
...
func (h *InvestingHandlers) GetPortfolio(c *gin.Context) {
	...
	if err != nil {
		h.logger.Error("Failed to get portfolio",
			"error", err,
			"user_id", userUUID)
		common.SendInternalError(c, "PORTFOLIO_ERROR", "Failed to retrieve portfolio")
		return
	}

	common.SendSuccess(c, portfolio)
}
...
func (h *InvestingHandlers) handleOrderError(c *gin.Context, err error, userUUID uuid.UUID) {
	...
	case investing.ErrBasketNotFound:
		common.SendBadRequest(c, common.ErrCodeBasketNotFound, "Specified basket does not exist")
	case investing.ErrInvalidAmount:
		common.SendBadRequest(c, common.ErrCodeInvalidAmount, "Invalid order amount")
	case investing.ErrInsufficientFunds:
		common.SendForbidden(c, common.ErrCodeInsufficientFunds)
	case investing.ErrInsufficientPosition:
		common.SendBadRequest(c, common.ErrCodeInsufficientPosition, "Insufficient position for sell order")
	default:
		h.logger.Error("Failed to create order",
			"error", err,
			"user_id", userUUID)
		common.SendInternalError(c, "ORDER_ERROR", "Failed to create order")
	}
}
