...
func (h *APIKeyHandlers) CreateAPIKey(c *gin.Context) {
	...
	if err != nil {
		...
		common.SendInternalError(c, common.ErrCodeInternalError, "Failed to create API key")
		return
	}
...
func (h *APIKeyHandlers) ListAPIKeys(c *gin.Context) {
	...
	if err != nil {
		...
		common.SendInternalError(c, common.ErrCodeInternalError, "Failed to list API keys")
		return
	}

	common.SendSuccess(c, gin.H{"api_keys": keys})
}
...
func (h *APIKeyHandlers) RevokeAPIKey(c *gin.Context) {
	...
	if err != nil {
		common.SendBadRequest(c, common.ErrCodeInvalidID, "Invalid key ID")
		return
	}
	...
	common.SendSuccess(c, gin.H{"message": "API key revoked"})
}
...
func (h *APIKeyHandlers) UpdateAPIKey(c *gin.Context) {
	...
	common.SendSuccess(c, gin.H{"message": "API key updated"})
}
...
func (h *APIKeyHandlers) GetAPIKey(c *gin.Context) {
	...
	for _, key := range keys {
		if key.ID == keyID {
			common.SendSuccess(c, key)
			return
		}
	}

	common.SendNotFound(c, common.ErrCodeNotFound, "API key not found")
}
...
func (h *AdminAPIKeyHandlers) AdminListAPIKeys(c *gin.Context) {
	...
	if userIDStr := c.Query("user_id"); userIDStr != "" {
		...
		if err != nil {
			common.SendBadRequest(c, common.ErrCodeInvalidUserID, "Invalid user ID")
			return
		}
		userID = &parsed
	}
...
func (h *AdminAPIKeyHandlers) AdminGetAPIKeyStats(c *gin.Context) {
	...
	if err != nil {
		...
		common.SendInternalError(c, common.ErrCodeInternalError, "Failed to get API key stats")
		return
	}
