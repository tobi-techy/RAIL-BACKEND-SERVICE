...
func (h *SessionHandlers) GetSessions(c *gin.Context) {
	...
	if err != nil {
		...
		common.SendInternalError(c, common.ErrCodeInternalError, "Failed to get sessions")
		return
	}
...
func (h *SessionHandlers) InvalidateSession(c *gin.Context) {
	...
	if token == "" {
		common.SendBadRequest(c, common.ErrCodeInvalidRequest, "Authorization header required")
		return
	}

	if err := h.sessionService.InvalidateSession(ctx, token); err != nil {
		h.logger.Error("Failed to invalidate session", zap.Error(err))
		common.SendInternalError(c, common.ErrCodeInternalError, "Failed to invalidate session")
		return
	}

	common.SendSuccess(c, gin.H{"message": "Session invalidated"})
}
...
func (h *SessionHandlers) InvalidateAllSessions(c *gin.Context) {
	...
	if err := h.sessionService.InvalidateAllUserSessions(ctx, userID); err != nil {
		...
		common.SendInternalError(c, common.ErrCodeInternalError, "Failed to invalidate sessions")
		return
	}

	common.SendSuccess(c, gin.H{"message": "All sessions invalidated"})
}
...
func (h *SessionHandlers) InvalidateSessionByID(c *gin.Context) {
	...
	if sessionID == "" {
		common.SendBadRequest(c, common.ErrCodeInvalidRequest, "Session ID is required")
		return
	}
	...
	if err != nil {
		h.logger.Error("Failed to get user sessions", zap.Error(err), zap.String("user_id", userID.String()))
		common.SendInternalError(c, common.ErrCodeInternalError, "Failed to validate session ownership")
		return
	}
	...
	if !sessionFound {
		common.SendNotFound(c, common.ErrCodeNotFound, "Session not found or does not belong to user")
		return
	}
...
func (h *SessionHandlers) GetActiveSessionCount(c *gin.Context) {
	...
	if err != nil {
		h.logger.Error("Failed to get user sessions", zap.Error(err), zap.String("user_id", userID.String()))
		common.SendInternalError(c, common.ErrCodeInternalError, "Failed to get session count")
		return
	}
...
func (h *SessionHandlers) GetSessionDetails(c *gin.Context) {
	...
	if err != nil {
		h.logger.Error("Failed to get user sessions", zap.Error(err), zap.String("user_id", userID.String()))
		common.SendInternalError(c, common.ErrCodeInternalError, "Failed to get session details")
		return
	}

	for _, s := range sessions {
		if s.ID.String() == sessionID {
			common.SendSuccess(c, s)
			return
		}
	}

	common.SendNotFound(c, common.ErrCodeNotFound, "Session not found")
}
