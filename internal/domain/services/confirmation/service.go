...
type Service struct {
	...
	// demoEmailOTP enables the hackathon email-OTP approve path and rejects
	// Face ID / token-only Approve (fail-closed). Default off.
	demoEmailOTP bool
	emailSender  EmailSender
	userEmails   UserEmailLookup
	otpStore     OTPStore
	otpSentAt    map[uuid.UUID]time.Time
	otpAttempts  map[uuid.UUID]int
...
func (s *Service) ApproveWithDevice(ctx context.Context, userID, id uuid.UUID, token, biometric string, dev DeviceApproval) (*entities.Confirmation, error) {
	...
	// Hackathon/demo: Face ID + token-only approve are rejected when email OTP
	// is the required path. Endpoint stays registered; clients must use OTP.
	if s.demoEmailOTP {
		return nil, fmt.Errorf("email OTP approval required (CONFIRMATION_DEMO_EMAIL_OTP): POST /api/v1/confirmations/{id}/otp/approve")
	}
