...
func TestPolicyVerdicts(t *testing.T) {
	cases := []struct {
		...
		{
			name:    "tier below advanced may still create a strategy",
			profile: &UserProfile{KYCTier: "basic", KYCStatus: "approved", Country: "NG"},
			action:  PolicyActionCreateStrategy,
			amount:  decimal.NewFromInt(50),
			want:    entities.InvestmentVerdictRequiresConfirmation,
		},
		{
			name:    "unverified user may still enroll",
			profile: &UserProfile{KYCTier: "non_kyc", KYCStatus: "pending", Country: "NG"},
