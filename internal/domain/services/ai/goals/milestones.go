...
// CheckSavingsMilestoneDeps holds the dependencies for checking savings milestones.
type CheckSavingsMilestoneDeps struct {
	GetAccountBalance func(ctx context.Context, userID uuid.UUID, accountType entities.AccountType) (decimal.Decimal, error)
}
...
func CheckSavingsMilestone(deps CheckSavingsMilestoneDeps, ctx context.Context, userID uuid.UUID, prevBalance decimal.Decimal) *MilestoneAlert {
	if deps.GetAccountBalance == nil {
		return nil
	}

	currentBalance, err := deps.GetAccountBalance(ctx, userID, entities.AccountTypeStashBalance)
	if err != nil {
		return nil
	}
