...
// IsAuditReady is the exported wrapper for isAuditReady.
func IsAuditReady(state *entities.MiriamMoneyState, hasBankStatement bool) bool {
	return isAuditReady(state, hasBankStatement)
}

// EstimateRateFromObligation is the exported wrapper for estimateRateFromObligation.
func EstimateRateFromObligation(d entities.FinancialObligation) decimal.Decimal {
	return estimateRateFromObligation(d)
}

// FilterToxicDebts is the exported wrapper for filterToxicDebts.
func FilterToxicDebts(debts []entities.FinancialObligation) []entities.FinancialObligation {
	return filterToxicDebts(debts)
}

// ToxicDebtThreshold is the interest rate above which a debt is considered toxic.
const ToxicDebtThreshold = 0.25
