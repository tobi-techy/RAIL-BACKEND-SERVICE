...
// PersonalityModeDeps holds dependencies for building personality mode context.
type PersonalityModeDeps struct {
...
func BuildPersonalityModeContext(deps PersonalityModeDeps, ctx context.Context, userID uuid.UUID) string {
	...
	profile, err := deps.GetToneProfile(fetchCtx, userID)
