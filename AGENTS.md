...
- iMessage bridge uses real `spectrum-ts` SDK (12.8.0, exact pin in `cmd/spectrum-bridge/package.json`), not the fake `@spectrumlabs/photon` package. Pattern: `Spectrum({projectId, projectSecret, providers:[imessage.config()]})` + webhook transport (`app.webhook()`, default `SPECTRUM_TRANSPORT_MODE=webhook`) with the streaming iterator (`for await (const [space, message] of app.messages)`) reserved for local `stream` mode — never both in prod (double at-least-once delivery). No `.spaces.get()`, no `.on("app.messages")` — real SDK is async-iterator based. Never patch `node_modules/@spectrum-ts/*/dist` (a postinstall dist patch was removed — it silently broke on every SDK bump); fix app-layer guards instead and report server bugs to Photon. Shutdown owns `await agent.stop()` before persisting `spaces.json`/outbound-queue and exiting. Sends route via `SpaceResolver` (`im.space.get` rehydration with persisted `{platform, phone}` meta — `phone` is required with 2+ dedicated lines), through the `OutboundPacer` token bucket, under the `DeliverabilityTracker` caps (5,000/server/day, 50 new convos/line/day).
...
- BRIJ x402 payments: the live `PAYMENT-REQUIRED` challenge sends `resource` as an OBJECT and `amount` as a STRING, which used to abort every paid call in the Go decoder (`acceptedPayment.Amount` is now `x402Amount`, a string-or-number int64 type; `paymentRequirement.Resource` is `json.RawMessage`). Two masked bugs were uncovered once parsing worked: `GET /air/orders/{id}` was an UNPAID call but the endpoint is paid ($0.01, needs the support code) — PNR was always empty; and `priorityFee = 0` makes `SetComputeUnitPriceInstructionBuilder().SetMicroLamports(0)` fail validation, so the price instruction is now skipped when 0. Browser-tier fares (`trip.com:`, `ryanair:` offer prefixes) require a fresh `/air/offer-details` drill before intent creation (retry once on 409 `fare_menu_expired`) and passport fields (`nationality`, `passport_number`, `passport_expiry`) at `/air/book` — Rail validates them before payment and threads them from saved `travel_passengers` profiles. `travel_orders.passenger_count` is persisted from the intent (migration 316) and `BookFlight` rejects multi-passenger intents before holding funds; funding-wallet USDC balance is preflighted before `Book` (fail-open on RPC error, hard-stop on short balance). `SearchFlights` now sends `limit=20` + `cheapest_per_itinerary=true` and `OfferSummary` carries `fare_brand_name`/`cabin_class`/bags/`conditions` so the model can distinguish fares instead of denying a fare's existence from a capped view.
- Spectrum webhook wraps unmapped inbound arms as `custom`, and `custom` used to mean "unreadable bubble" in the bridge — so every iMessage poll tap (onboarding answer selections, and Confirm/Cancel on poll-less channels) reached `inbound.ts` looking like a sticker and the user got "I can't open that kind of message" instead of their answer being counted. `spectrum-ts`'s native-webhook `mapContent` (core `dist`, `deserializeContent`) has no `poll_option` case — Photon does deliver poll responses as `poll_option` content ("Polls" docs), and `asCustom(content)` preserves the original payload under `.raw`. The bridge now calls `recoverCustomContent(raw)` on `custom` (allowlist of router-handled arms the SDK does NOT map: `poll_option`, `poll`, `typing`, `read`, `unsend`, `edit`, `effect`, `markdown`, `app`) and re-enters `routeInboundContent`; arms the SDK does map are deliberately NOT recovered, because when one of those arrives as `custom` its own parser threw and re-routing would throw again. The notice path now logs `raw_type`/`imessage_type`/`raw_keys` so an unreadable bubble is identifiable without a redeploy. Fix stays app-layer — never patch `node_modules` (see the postinstall-patch lesson above).
- The iMessage provider ALSO mints `custom` with `raw: { imessage_type: "unsupported-message" }` for any no-text/no-attachment event — read receipts, delivered receipts, stickers, handwriting. Production VM logs (Sep 25) showed bursts of these every ~6-7s, matching the user reading each paced outbound bubble; each one triggered a spurious "I can't open that kind of message" reply, including one before the user had even said "Hey". The sentinel carries zero user content, so `isNoContentEvent(raw)` now acks it silently (logged with diagnostics, no backend post). Structured unknown customs (real fields, not the sentinel) still get the courtesy notice. Webhook docs confirm: webhooks deliver inbound only (no outbound echoes), and the only documented inbound arms are text/attachment/contact/richlink/reaction/group — replies arrive as plain `text`, voice as `attachment`.
- The bridge legitimately forwards inbound **read receipts** (and group lifecycle events / retractions) to `/api/v1/platform/inbound` for delivery tracking, but the Go `platform.InboundMessage` never modelled `is_read_receipt`/`is_group_event`/`is_unsend`/`is_oversized` — so each one landed as an `text:""` turn. For an unlinked sender onboarding's empty-text fallback answered "I can't open that kind of message. Text me what you need."; the user then read that reply, the provider minted another receipt, and the conversation looped on the canned line forever (Sep 26 VM logs: read receipts every ~6s after "Hey Miriam"). Fix on the backend (app layer, no `node_modules` patching): `InboundMessage` now models those signals and `Processor.Process` acks read receipts/group events/unsends **before** any identity lookup or model turn; `is_oversized` gets a size-specific notice; and a `carriesUserContent()` guard drops any contentless payload (a bare tapback included) that would otherwise become an empty model call or onboarding fallback. Tests: `TestProcess_LifecycleSignalsAreAckedWithoutReply`, `TestProcess_ContentlessMessageIsAckedWithoutReply`, `TestProcess_OversizedAttachmentGetsSizeNotice`.
- Photon best-practice alignment (docs `best-practices/{architecture,inbound-pipeline,recovery-and-state,imessage-deliverability}`). Already in place: burst debounce, mark-as-read after reply, paced sends (typing + inter-bubble delays + cross-conversation `OutboundPacer`), inbound-first deep links, early contact-card share, first-message link suppression, hard caps (5,000 outbound/server/day + 50 new convos/line/day), per-user memory scope by container tag, and the `FailureAudit` log. Two gaps were closed. (1) **Carry-forward** (`inbound.ts`): a debounced batch that exhausts its retry budget used to be dropped, silently eating the user's words; it is now stored per-thread and prepended to that thread's next message as `[Earlier message] …` lines (`onCarried`, `hasCarried`, `carryForward:false` opts out; `flushAll` best-effort reposts carried batches on shutdown). (2) **Bubble-level idempotency** (`handler.ts`): a `client_guid` used to be marked seen on *attempt*, so every outbound-queue retry of a provider-failed reply was a silent no-op and a crash mid-bubble discarded the rest. Now `handleOutbound` records completion only after the whole reply is accepted, and each bubble carries a stable `${client_guid}#<i>` key recorded only after the provider acks it (`sendBubble`) — a retry resumes from the first unsent bubble. The 2s text-window dedup now applies only to payloads without a guid. Tests: `carry-forward` cases in `inbound.test.ts`, `MessageHandler idempotent retries` in `handler.test.ts`. (3) **Durable debounce** (`inbound-spool.ts`): the debounce buffer used to be in-memory, so a hard crash mid-quiet-window lost the user's words. `InboundDebouncer.snapshot()/restore()` plus a JSON spool (`data/inbound-spool.json`, `startAutoSave`, flush on shutdown) re-arm buffered + carried batches on boot; a batch whose deadline passed while down flushes immediately. `/health` reports `inbound_spool`. Tests: `inbound-spool.test.ts` + snapshot/restore cases in `inbound.test.ts`. (4) **In-flight supersession** — see the next bullet.
- In-flight turn supersession (Photon `architecture`: cancel a stage when a follow-up lands). A newer message arriving while a reply is generating used to ship both answers. Two flag-gated gates now drop the stale one without aborting the HTTP call and without touching committed state. The bridge mints `turn_id` per inbound batch in `postToBackend` **only for content-bearing payloads** (`turn-supersession.ts`; read receipts, unsends, group events and unsupported/oversized notices are deliberately left untagged — a read receipt arrives constantly while we generate, and letting it become "the latest turn" would silently swallow the answer to the message the user actually sent). Tagging is idempotent, so a retried flush reuses its original turn. The Go `TurnTracker` records the newest turn per conversation in Redis (`miriam:turn:<platform>:<thread>`, 5m TTL — longer than the slowest statement-scan turn, short enough not to pin a long-idle thread), and `Processor.send`/`Processor.sendToSender` stamp it onto every reply and drop the reply when superseded (**gate 1**); the bridge's `sendToSpace` drops any `/send` whose `turn_id` is no longer the thread's latest (**gate 2**), which is also the only path a queued replay takes. Only those two helpers are gated — the transient typing indicator, `ProcessAction` confirm/cancel acknowledgements and proactive `BridgeDispatcher` sends run with no turn on the context and always deliver, so a confirmed money action's receipt is never suppressed. Everything fails open (no tracker, no turn, unknown thread, Redis error, disabled flag ⇒ deliver); the bridge's turn map is bounded (`MAX_TRACKED_TURNS`, evicted by the existing 60s sweeper) and clearing it only disables suppression until the next message. Flags: `MIRIAM_TURN_SUPERSESSION` (bridge) and `PLATFORM_TURN_SUPERSESSION` (backend, `platform.turn_supersession`), both off by default and safe in any combination — off means no turn id is even minted. Gate 1 gates *delivery* only: a superseded turn still pays for its model pass (and its statement scan), because the scan is what produces the reply and a staged confirmation must not be auto-cancelled — the design's pre-emptive "don't start a side effect" step is a documented non-goal. Tests: `TestProcess_SupersededTurnDoesNotDeliver`, `TestProcess_CurrentTurnDeliversAndStampsTurnID`, `TestProcessAction_NeverSuperseded`, `TestTurnTracker_NilRedisFailsOpen`, plus `turn-supersession.test.ts`. Design + implementation notes: `docs/miriam-inbound-supersession.md`.
